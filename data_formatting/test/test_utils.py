...
def test_trains_path():
    ...
    assert get_J(train_dict) == [94766, 26103, 421009, 42100, 5312, 40518, 34319, 343199, 14006, 94611, 40150, 41004, 94113, 40673, 54101, 541019, 40477, 4500, 94317, 44717, 64350, 94717, 44862, 40628, 40675, 73000, 4120]
...
def test_path_reffering_to_stations():
    ...
    assert get_blocks_b2win_station4train(train_dict[train1][1], "MJ", "Mi") == (['"MJ-Mi", "Sem(odstep)", 1, "1", "(1)"'], False)
    assert get_common_blocks_and_direction_b2win_trains(time_tables_dict,train1, train2,"MJ", "Mi") == (['"MJ-Mi", "Sem(odstep)", 1, "1", "(1)"'], 'opposite')

    # trains departuring from particular station
    assert get_trains_at_station(time_tables_dict,important_stations, True)["Mi"] == [44717, 44862]
    ...
    assert get_trains_at_station(train_dict,important_stations)['CB'] == [26103, 5312, 40150, 40673, 4500, 94317, 64350, 40628, 40675, 73000]
