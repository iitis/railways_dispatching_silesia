...
def get_blocks_b2win_station4train(timetable, station1, station2, verbose=True):
    ...
    if sts.index(station1) > sts.index(station2):
        ...
        df = timetable
        mask = (df["important_station"] == station1) | (
            df["important_station"] == station2
        )
...
def get_blocks_b2win_station4train_deprecable(
    ...
    if station2 not in sts and station1 not in sts:
        print(
            "Warning: station {} and {} not in the train set".format(station1, station2)
        )
        return [], None
...
def get_common_blocks_and_direction_b2win_trains(
    time_tables_dict, train1, train2, station1, station2, verbose=False
):
    blocks_order = {}
    for train in [train1, train2]:
        blocks, rev = get_blocks_b2win_station4train(
            time_tables_dict[train][1], station1, station2, verbose
        )
        if rev:
            blocks, _ = get_blocks_b2win_station4train(
                time_tables_dict[train][1], station2, station1, verbose
            )
        blocks_order[train] = [blocks, rev]
    common_blocks = check_common_specific_elements_lists(
        blocks_order[train1][0], blocks_order[train2][0]
    )
...
def common_path(timetable1, timetable2, station1, station2, verbose=False):
    ...
    if blocks1 == blocks2:
        ...
        common_path_list = flatten(
            check_common_specific_elements_lists(blocks1, blocks2)
        )
...
def subsequent_block(time_table_blocks, block, verbose=False):
    ...
    assert (
        block in time_table_blocks
    ), f"this train does not pass trought this block: {block}"
    ...
    for station in important_stations.keys():
        ...
        for train in trains_infor.keys():
            if only_departue == False:
                if (
                    trains_infor[train][1]["path"]
                    .isin(important_stations[station])
                    .any()
                ):
...
def minimal_passing_time(timetable, station1, station2, resolution=1, verbose=False):
    ...
    blocks, _ = get_blocks_b2win_station4train(
        timetable, station1, station2, verbose=verbose
    )
