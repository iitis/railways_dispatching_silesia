...
from .utils import (
    ...
    flatten,
    get_J,
    get_passing_time_block,
    minimal_passing_time,
    minimal_stay,
    subsequent_block,
    turn_around_time,
)
...
def get_jround(train_dict, important_stations):
    ...
    for pair in pair_lists:
        a = (
            train_time_table(train_dict, pair[0])["path"].tolist()[0]
            == train_time_table(train_dict, pair[1])["path"].tolist()[-1]
        )
        b = (
            train_time_table(train_dict, pair[1])["path"].tolist()[0]
            == train_time_table(train_dict, pair[0])["path"].tolist()[-1]
        )
        if a:
...
def josingle_dict_generate(train_dict, j, j_prime, s, s_prime, init_josingle):
    ...
    path_j_prime = get_blocks_b2win_station4train(
        time_talbe_j_prime, s_prime, s, verbose=False
    )[0]
...
def important_trains_and_stations(
    trains_dict, important_stations, imp_stations, only_departue
):
...
def josingle(trains_dict, important_stations, imp_stations=None):
    ...
    imp_stations_list, trains_at_stations = important_trains_and_stations(
        trains_dict, important_stations, imp_stations, True
    )
        ...
        for j in trains_at_stations[s]:
            ...
            if s_prime in imp_stations_list:
                for j_prime in trains_at_stations[s]:
                    if j_prime != j and s == subsequent_station(
                        trains_dict[j_prime][1], s_prime
                    ):
                        if (s_prime, s) not in init_josingle.keys():
                            josingle_dict_generate(
                                trains_dict, j, j_prime, s, s_prime, init_josingle
                            )
...
def jtrack(trains_dict, important_stations, imp_stations=None):
    ...
    init_jtrack = {}
    imp_stations_list, trains_at_stations = important_trains_and_stations(
        trains_dict, important_stations, imp_stations, False
    )
...
def make_jswitch_dict(
    train_dict,
    s,
    j,
    jprime,
    in_switch_sequence_j,
    in_switch_sequence_jprime,
    out_switch_sequence_j,
    out_switch_sequence_jprime,
    vec_of_pairs,
    non_repeating_pair,
):
    ...
    if bool(in_switch_sequence_j.intersection(in_switch_sequence_jprime)) != False:
        if s != non_repeating_pair[1]:
            ...
            subsequent_station(train_dict[j][1], non_repeating_pair[0]) != s
            and subsequent_station(train_dict[jprime][1], non_repeating_pair[0]) != s
        ):
            vec_of_pairs.append({j: "in", jprime: "in"})
    ...
    if bool(in_switch_sequence_j.intersection(out_switch_sequence_jprime)) != False:
        if s != non_repeating_pair[1]:
            ...
            subsequent_station(train_dict[j][1], non_repeating_pair[0]) != s
            and subsequent_station(train_dict[jprime][1], s) != non_repeating_pair[0]
        ):
            vec_of_pairs.append({j: "in", jprime: "out"})
    ...
    if bool(out_switch_sequence_j.intersection(in_switch_sequence_jprime)) != False:
        if s != non_repeating_pair[1]:
            ...
            subsequent_station(train_dict[j][1], s) != non_repeating_pair[0]
            and subsequent_station(train_dict[jprime][1], non_repeating_pair[0]) != s
        ):
            vec_of_pairs.append({j: "out", jprime: "in"})
...
def jswitch(train_dict, important_stations, data_switch, imp_stations=None):

    jswitch = {}

    imp_stations_list, trains_at_stations = important_trains_and_stations(
        train_dict, important_stations, imp_stations, False
    )
    ...
    for s in imp_stations_list:
        ...
        for j, jprime in itertools.combinations(trains_at_stations[s], 2):
            in_switch_sequence_j = z_in(
                data_switch, j, s, paths, station_block, blocks_list
            )
            out_switch_sequence_j = z_out(
                data_switch, j, s, paths, station_block, blocks_list
            )
            in_switch_sequence_jprime = z_in(
                data_switch, jprime, s, paths, station_block, blocks_list
            )
            out_switch_sequence_jprime = z_out(
                data_switch, jprime, s, paths, station_block, blocks_list
            )
        ...
        for j in trains_at_stations[s]:
            ...
            while j not in flatten(jd[s][s2]):
                ...
                while i < len(jd[s][s2]):
                    path_check = (
                        common_path(
                            timetable, time_tables_dict[jd[s][s2][i][0]][1], s, s2
                        )
                        == get_blocks_b2win_station4train(timetable, s, s2)[0]
                    )
...
def get_taus_headway(train_dict: dict, important_stations: dict, r=1):
    ...
    for station1 in important_stations_list:
        for station2 in jd_dict[station1].keys():
            for train1, train2 in [
                a
                for a in it.product(flatten(jd_dict[station1][station2]), repeat=2)
                if a[0] != a[1]
            ]:
                blocks_sequence = common_path(
                    train_dict[train1][1], train_dict[train2][1], station1, station2
                )
                t_pass = np.zeros(len(blocks_sequence))
                deltas_vec = np.zeros(len(blocks_sequence))
                for i in range(len(blocks_sequence)):
                    t = lambda i, train: get_passing_time_block(
                        blocks_sequence[i], train_dict[train][1]
                    )
                    t_pass[i] = t(i, train1)
                    if i > 0:
                        deltas_vec[i] = sum(
                            [t(j, train1) - t(j, train2) for j in range(i)]
                        )
                    k = 2
                    if (station1, station2) in list_of_k1_pairs or (
                        station2,
                        station1,
                    ) in list_of_k1_pairs:
                        k = 1
                ...
                if len(blocks_sequence) > 0:
                    t_headway = np.max(
                        [
                            np.sum([t_pass[x : x + k]]) + deltas_vec[x]
                            for x in range(len(deltas_vec))
                        ]
                    )
                if r == 1:
