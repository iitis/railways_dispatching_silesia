...
for j in [3,3.5,4,4.5]:
    with open('files/dwave_data/Qfile_samples_sol_real-anneal_numread3996_antime250_chainst{}'.format(j), 'rb') as fp:
        x = pickle.load(fp)
...
print('Reroute:')
for j in [3,3.5,4,4.5]:
    with open('files/dwave_data/Qfile_samples_sol_real-anneal_numread3996_antime250_chainst{}_r'.format(j), 'rb') as fp:
        x = pickle.load(fp)
        print(x[0][1], 'chain strenght', j)
